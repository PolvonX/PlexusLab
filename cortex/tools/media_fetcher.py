...
class MediaFetcherTool(BaseTool):
    ...
    async def execute(self, action_args: dict[str, Any], ctx: Any) -> ToolResult:
        ...
        log.info(f"Запуск yt-dlp для URL: {url}")
        ...
        try:
            ...
            if process.returncode != 0:
                log.error(f"Ошибка yt-dlp: {stderr}")
            ...
            return ToolResult.failure(
                "Утилита yt-dlp не найдена в системе. Убедитесь, что она установлена (pip install yt-dlp) и доступна в PATH."
            )
